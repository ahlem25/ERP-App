package com.iss4u.erp.services.controller;

import org.springframework.web.bind.annotation.*;
import com.iss4u.erp.services.service.CommandeClientService;
import com.iss4u.erp.services.modules.vente.domain.billing.dto.commandeclient.request.CommandeClientRequest;
import com.iss4u.erp.services.modules.vente.domain.billing.dto.commandeclient.response.CommandeClientResponse;
import java.util.List;
import java.util.Optional;
import lombok.RequiredArgsConstructor;

@RestController
@RequiredArgsConstructor
@RequestMapping("/api/v1/commandes-client")
public class CommandeClientController {

    private final CommandeClientService service;

    @GetMapping
    public List<CommandeClientResponse> findAll() {
        return service.findAll();
    }

    @GetMapping("/{id}")
    public Optional<CommandeClientResponse> findById(@PathVariable Long id) {
        return service.findById(id);
    }

    @PostMapping
    public CommandeClientResponse create(@RequestBody CommandeClientRequest request) {
        return service.save(request);
    }

    @PutMapping("/{id}")
    public CommandeClientResponse update(
        @PathVariable Long id,
        @RequestBody CommandeClientRequest request
    ) {
        return service.update(id, request);
    }

    @DeleteMapping("/{id}")
    public void delete(@PathVariable Long id) {
        service.delete(id);
    }
}