package com.iss4u.erp.services.controller;

import org.springframework.web.bind.annotation.*;
import com.iss4u.erp.services.service.FactureAchatService;
import com.iss4u.erp.services.modules.achat.domain.commande.dto.factureachat.request.FactureAchatRequest;
import com.iss4u.erp.services.modules.achat.domain.commande.dto.factureachat.response.FactureAchatResponse;
import java.util.List;
import java.util.Optional;
import lombok.RequiredArgsConstructor;

@RestController
@RequiredArgsConstructor
@RequestMapping("/api/v1/factures-achats")
public class FactureAchatController {

    private final FactureAchatService service;

    @GetMapping
    public List<FactureAchatResponse> findAll() {
        return service.findAll();
    }

    @GetMapping("/{id}")
    public Optional<FactureAchatResponse> findById(@PathVariable Long id) {
        return service.findById(id);
    }

    @PostMapping
    public FactureAchatResponse create(@RequestBody FactureAchatRequest request) {
        return service.save(request);
    }

    @PutMapping("/{id}")
    public FactureAchatResponse update(
        @PathVariable Long id,
        @RequestBody FactureAchatRequest request
    ) {
        return service.update(id, request);
    }

    @DeleteMapping("/{id}")
    public void delete(@PathVariable Long id) {
        service.delete(id);
    }
}